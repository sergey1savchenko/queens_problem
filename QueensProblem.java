...
public class QueensProblem {
	...
	public static void main(String[] args){
		...
		while (chessboard[0]<N) {							// для всех столбцов поля
			
			if (checked()){									// если на данный момент нет ферзей под боем
				
				if (index == (N-1)) {						// если мы уже заполнили все строки
					System.out.print(++version + ": ");
					printResult(chessboard);
					chessboard[index]++;
				}
				else {										// если рассмотрели еще не все строки 
					index++;								// переходим на следующую
				}
				
			}
			else {											// проверка не пройдена => сдвигаем ферзя на текущей позиции вверх
				chessboard[index]++;
			}
